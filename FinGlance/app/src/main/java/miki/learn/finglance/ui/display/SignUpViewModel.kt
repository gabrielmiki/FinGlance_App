...
class SignUpViewModel(
    ...
    suspend fun signUpUser() {
        val result = accountRepository.signUpEmailPassword(email, password)

        onSignUpResult(result)
    }

    fun storeUserInRealTimeDatabase() {
        val signedUser = accountRepository.getSignedInUser()

        realtimeStorageRepository.createFirebaseUserData(signedUser)
    }
